...
<div id="main" class="wrap">
	<div class="lefter">
		...
		<div class="main">
		<div class="guestbook">
		<form name="listForm" action="CommentServlet?action=queryq" method="post">
			<h2>全部留言</h2>
			<ul>
				<c:forEach items="${ph.list}" var="ly">
				<li>
					<dl>
						<dt>${ly.econten}</dt>
						<dd class="author">网友：${ly.ecnicename} <span class="timer">${ly.ecrettime}</span></dd>
						<c:if test="${ly.ecreply==null}">
						<dd>管理员暂未回复</dd>
						</c:if>
						<c:if test="${ly.ecreply!=null}">
						<dd>管理员：${ly.ecreply }<span class="timer">${ly.ecreplytime}</span></dd>
						</c:if>
					</dl>
				</li>
				</c:forEach>
