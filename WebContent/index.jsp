<%@ page language="java"
	import="easybuy.dao.*,easybuy.dao.imp.*,easybuy.entity.*,java.util.*"
...
<body>
	...
	if("${listpdt}"==""){
		window.location.href = "LoctionServlet?action=query";
	}
	</script>
	<div id="header" class="wrap">
		...
		<div>${login}</div>
			<a href="CartServlet?action=query&uesr_id=${message.uid}" class="shopping">购物车</a><a href="login.jsp">登录</a><a
				href="register.jsp">注册</a><a href="CommentServlet?action=queryq">留言</a>
		</div>
		<div class="navbar">
			<ul class="clearfix">
		...
		<div class="main">
			<div class="price-off">
				...
				<ul class="product clearfix">
					<c:forEach items="${listpdt}" var="p">
						<li>
							<dl>
								<dt>
									<a href="LoctionServlet?action=view&id=${p.epid}" target="_blank"><img
										src="/index/${p.ul}" /></a>
								</dt>
								<dd class="title">
									<a href="LoctionServlet?action=view&id=${p.epid}" target="_blank">${p.epname}</a>
								</dd>
				...
				<div class="news-list">
					...
					<ul>
						<c:forEach items="${listnew}" var="n">
							<li><a href="news-view.jsp" target="_blank">${n.title}</a></li>
						</c:forEach>
					</ul>
				</div>
				<div class="spacer"></div>
