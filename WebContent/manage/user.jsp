...
<body>
	<div id="header" class="wrap">
		...
		<div class="navbar">
			<ul class="clearfix">
				...
				<li class="current"><a href="/Easybuy/UserServlet?action=query">用户</a></li>
	...
	<form name="listForm" action="UserServlet?action=query" method="post">
			...
			<div class="main">
				...
				<div class="manage">
					<table class="list">
						...
						<c:forEach items="${ph.list}" var="yh">
							<tr>
								<td class="first w4 c">${yh.uid}</td>
								<td class="w1 c">${yh.uname}</td>
								<td class="w2 c">${yh.sex}</td>
								<td>${yh.email}</td>
								<td class="w4 c">${yh.phone}</td>
								<td class="w1 c"><a
									href="UserServlet?action=get&id=${yh.uid}">修改</a><a
									href="UserServlet?action=delete&id=${yh.uid}">删除</a></td>
							</tr>
						</c:forEach>
